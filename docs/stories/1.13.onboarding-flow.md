...
**⚠️ CRITICAL AC UPDATE from User Flows v2.1**:
...
- [ ] Step 5: Create `/app/onboarding/goals/page.tsx`
  ...
  - [ ] Multi-select cards (2×3 grid)
  - [ ] Checkboxes in top-right corner
  - [ ] Next button (enabled after ≥1 selection) → Step 6
...
- [ ] Step 6: Create `/app/onboarding/equipment/page.tsx`
  ...
  - [ ] Multi-select cards (2×3 grid)
  - [ ] Next button (enabled after ≥1 selection) → Step 7
...
- [ ] Step 8a: Create `/app/onboarding/describe-program/page.tsx`
  ...
  - [ ] Voice input button (🎤) for transcription
  - [ ] Character counter
  - [ ] Primary CTA: "Create My Program" (AI processing)
  - [ ] Secondary link: "Use Program Builder instead" → Program Builder
...
- [ ] Add Back arrow to all steps except Step 1
  - [ ] Top-left corner
  - [ ] Icon: Left arrow
  - [ ] Click → Navigate to previous step
...
- [ ] Write unit tests for user state branching
  - [ ] Test: New user → Redirects to onboarding
  - [ ] Test: Existing user → Redirects to dashboard (skips onboarding)
  - [ ] Test: Unknown state → Defaults to dashboard (fail-safe)
- [ ] Write integration tests for onboarding flow
  - [ ] Test: Complete all 9 steps → Dashboard
  - [ ] Test: Skip at Step 3 → Dashboard
...
- [ ] Write E2E test for complete onboarding
  - [ ] Test: Signup → Onboarding (9 steps) → Program setup → Dashboard
  - [ ] Test: Login (existing user) → Dashboard (no onboarding)
...
│         │     Describe it and we'll set it up for you               │     │
│         │                                                            │     │
│         │                                                       [→]  │     │
│         └────────────────────────────────────────────────────────────┘     │
│                                                                              │
│         ┌────────────────────────────────────────────────────────────┐     │
│         │  📋 No, I need a program                                   │     │
│         │     We'll suggest programs based on your goals             │     │
│         │                                                            │     │
│         │                                                       [→]  │     │
│         └────────────────────────────────────────────────────────────┘     │
│                                                                              │
...
**Buttons**:
- Next button: Full-width, 48px height, orange (#FF6B2C), white text
- Skip link: Top-right, 14px, orange (#FF6B2C), underline on hover
...
- Current dot: Filled orange (#FF6B00) or blue (#0B87D9)
...
- **Next Button**: Orange (#FF6B00), white text, 56px height, rounded 12-16px, full-width
- **Skip Link**: Orange (#FF6B2C) text, 14px, top-right, underline on hover
...
- Signup → Onboarding → Program setup → Dashboard
- Login (existing user) → Dashboard (no onboarding)
...
- E2E test: Full user journey (signup → onboarding → dashboard)

### Test Scenarios
1. **New User Complete Flow**: Signup → 9 steps → Program setup → Dashboard
2. **Existing User Skip**: Login → User state detected → Skip to dashboard (no onboarding)
3. **Skip at Step 3**: Start → Step 1, 2, 3 → Skip → Dashboard
4. **Back Navigation**: Step 5 → Back → Step 4 (answers preserved)
5. **Multi-Select**: Goals step → Select 3 goals → Saved correctly
6. **Program Path A**: "Yes, I have program" → Describe → AI creates → Dashboard
7. **Program Path B**: "No program" → Recommendations → Select → Dashboard
8. **Program Path C**: "Skip" → Dashboard empty state
